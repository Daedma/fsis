...
public:
    ...
    sf::Socket::Status registerOnServer(sf::IpAddress serverIp, unsigned short serverRegPort, std::string clientName, std::string password);

    /*Receives and records connected clients names to passed vector*/
    sf::Socket::Status receiveConnectedClientsNames(std::vector<std::string>& namesVec);
...
private:
    ...
    sf::Socket::Status sendClientRecipientData(std::string clientName,  std::string password);
